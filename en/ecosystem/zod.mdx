---
title: "Zod Library"
description: "Use @rewritejs/zod for official, versioned Zod schemas for runtime validation across Rewrite integrations."
icon: shield-check
---

`@rewritejs/zod` provides official Zod schemas for the Rewrite API so you can validate requests, responses, reusable resources, and webhook payloads at runtime.
...
<CardGroup cols={2}>
  <Card title="Versioned schemas" icon="tag" color="#C9CBD1">
    Import schemas from namespaces such as `@rewritejs/zod/v1`.
  </Card>
...
<Accordion title="Keep versions explicit">
  Import from a specific API namespace such as `@rewritejs/zod/v1` so upgrades stay deliberate.
</Accordion>

<Accordion title="Pair runtime and transport layers">
  Combine `@rewritejs/zod` with `@rewritejs/rest` and `@rewritejs/types` for validation, routing, and transport in one stack.
</Accordion>
